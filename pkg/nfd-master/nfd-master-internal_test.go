	...
	Convey("When servicing SetLabels request", t, func() {
		...
		Convey("When node update succeeds", func() {
			expectedPatches := []apihelper.JsonPatch{
				...
				apihelper.NewJsonPatch("add", "/metadata/annotations", nfdv1alpha1.FeatureAnnotationsTrackingAnnotation, ""),
			}
		...
		Convey("When -extra-label-ns, -deny-label-ns and -instance are specified", func() {
			...
			expectedPatches := []apihelper.JsonPatch{
				...
				apihelper.NewJsonPatch("add", "/metadata/annotations", instance+"."+nfdv1alpha1.FeatureAnnotationsTrackingAnnotation, ""),
			}
