...
type nfdController struct {
	configLister     nfdlisters.WorkerConfigLister
	stopChan         chan struct{}
	updateConfigChan chan map[string]string
}
...
func newNfdController(config *restclient.Config, nfdApiControllerOptions nfdApiControllerOptions) (*nfdController, error) {
	c := &nfdController{
		...
		updateConfigChan: make(chan map[string]string),
	}
	...
	configInformer := nfdinformersv1alpha1.New(informerFactory, "", tweakListOpts).WorkerConfigs()
	if _, err := configInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			cfg := obj.(*nfdv1alpha1.WorkerConfig)
			klog.V(2).InfoS("WorkerConfig added", "workerconfig", klog.KObj(cfg))
			c.updateConfiguration(cfg)
		},
		UpdateFunc: func(oldObj, newObj interface{}) {
			cfg := newObj.(*nfdv1alpha1.WorkerConfig)
			klog.V(2).InfoS("WorkerConfig updated", "workerconfig", klog.KObj(cfg))
			c.updateConfiguration(cfg)
		},
		DeleteFunc: func(obj interface{}) {
			cfg := obj.(*nfdv1alpha1.WorkerConfig)
			klog.V(2).InfoS("WorkerConfig deleted", "workerconfig", klog.KObj(cfg))
			c.updateConfiguration(cfg)
		},
	}); err != nil {
		return nil, err
...
func (c *nfdController) updateConfiguration(cfg *nfdv1alpha1.WorkerConfig) {
	select {
	case c.updateConfigChan <- cfg.Spec.Data:
	case <-c.stopChan:
	}
}
