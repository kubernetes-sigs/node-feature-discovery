...
func MatchGetKeys(m *nfdv1alpha1.MatchExpressionSet, keys map[string]nfdv1alpha1.Nil) (bool, []MatchedElement, error) {
	...
	for n, e := range *m {
		...
		ret = append(ret, MatchedElement{MatchedKeyName: n})
	}
	// Sort for reproducible output
	sort.Slice(ret, func(i, j int) bool { return ret[i][MatchedKeyName] < ret[j][MatchedKeyName] })
	return true, ret, nil
...
func MatchValues(m *nfdv1alpha1.MatchExpressionSet, values map[string]string, runStrategy RunStrategy) (bool, error) {
	matched, _, err := MatchGetValues(m, values, runStrategy)
	return matched, err
}
...
// MatchExpressionSet returns a match with an empty slice of matched features.
func MatchGetValues(m *nfdv1alpha1.MatchExpressionSet, values map[string]string, runStrategy RunStrategy) (bool, []MatchedElement, error) {
	ret := make([]MatchedElement, 0, len(*m))
	isMatch := true
	...
	for n, e := range *m {
		...
		if match {
			ret = append(ret, MatchedElement{MatchedKeyName: n, MatchedKeyValue: values[n]})
		} else {
...
// (attributes).
func MatchInstances(m *nfdv1alpha1.MatchExpressionSet, instances []nfdv1alpha1.InstanceFeature, runStrategy RunStrategy) (bool, error) {
	isMatch, _, err := MatchGetInstances(m, instances, runStrategy)
	return isMatch, err
}
...
func MatchGetInstances(m *nfdv1alpha1.MatchExpressionSet, instances []nfdv1alpha1.InstanceFeature, runStrategy RunStrategy) (bool, []MatchedElement, error) {
	ret := []MatchedElement{}

	for _, i := range instances {
		if match, err := MatchValues(m, i.Attributes, runStrategy); err != nil {
			return false, nil, err
		} else if match {
			ret = append(ret, i.Attributes)
		}
...
func MatchMulti(m *nfdv1alpha1.MatchExpressionSet, keys map[string]nfdv1alpha1.Nil, values map[string]string, instances []nfdv1alpha1.InstanceFeature, runStrategy RunStrategy) (bool, []MatchedElement, error) {
	...
	ma, me, err := MatchGetInstances(m, instances, runStrategy)
	if err != nil {
		return false, nil, err
	}
	isMatch = isMatch || ma
