...
func Execute(r *nfdv1alpha1.Rule, features *nfdv1alpha1.Features, failFast bool) (RuleOutput, error) {
	...
	switch {
	case len(r.MatchFeatures) > 0 && len(r.MatchAny) > 0:
		matchStatus.IsMatch = isMatchAny && isMatchFeature
	case len(r.MatchFeatures) > 0:
		matchStatus.IsMatch = isMatchFeature
	case len(r.MatchAny) > 0:
		matchStatus.IsMatch = isMatchAny
	}
