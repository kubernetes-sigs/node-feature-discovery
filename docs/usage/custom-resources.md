...
grouping Nodes by matching features rules. NFD-Master watches for
NodeFeatureGroupRules objects in the cluster and creates a NodeFeatureGroup
object for each NodeFeatureRule.
...
The NodeFeatureGroup is disabled by default and can be enabled by setting the
`-enable-nodefeaturegroup-api` flag to `true` for nfd-master.
